...
constexpr int N_SPHERES  = 10;    // how many spheres in our vertical stack
...
__global__ void render_kernel(Vec *fb,
    ...
    for(int y = 0; y < HEIGHT; ++y) {
      for(int x = 0; x < WIDTH; ++x) {
        ...
        if (hit_id >= 0) {
          ...
          Vec V    = norm(-dir);              // now works!
          Vec Kd   = spheres[hit_id].color;
          Vec Ks   = {1.0f, 1.0f, 1.0f};
          float shin = 32.0f;
          pixelColor = phong_shade(P, N, V, lightPos, Kd, Ks, shin);
        }
        else {
...
int main() {
  ...
  // device alloc & copy
  Vec    *d_fb;      cudaMalloc(&d_fb,      WIDTH*HEIGHT * sizeof(Vec));
  Sphere *d_spheres; cudaMalloc(&d_spheres, N_SPHERES     * sizeof(Sphere));
  cudaMemcpy(d_spheres, h_spheres,
             N_SPHERES * sizeof(Sphere),
             cudaMemcpyHostToDevice);

  // render & download
  render_kernel<<<1,1>>>(d_fb, d_spheres, N_SPHERES);
  cudaDeviceSynchronize();
  Vec *h_fb = new Vec[WIDTH*HEIGHT];
